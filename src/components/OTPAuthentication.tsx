  ...
  const sendOtp = async () => {
    ...
    try {
      const response = await fetch(`${supabaseUrl}/functions/v1/otp-auth`, {
        ...
        body: JSON.stringify({ action: 'send-otp', mobile_number: mobileNumber }),
      });
  ...
  const verifyOtp = async () => {
    ...
    try {
      const response = await fetch(`${supabaseUrl}/functions/v1/otp-auth`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'apikey': supabaseAnonKey },
        body: JSON.stringify({ action: 'verify-otp', mobile_number: mobileNumber, code: otpCode, first_name: firstName, last_name: lastName }),
      });
      const data = await response.json();
        ...
        <div className="bg-white rounded-3xl shadow-xl p-8 border border-gray-100">
          <div className="text-center mb-6">
            ...
            <p className="text-gray-500 text-sm">
              {step === 'phone' ? 'Enter your mobile number to receive an OTP' : `Enter the 6-digit code sent to ${mobileNumber}`}
            </p>
          </div>
          ...
          {step === 'phone' && (
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={firstName} onChange={(e) => setFirstName(e.target.value)} placeholder="e.g., Rahul"
              ...
              <div>
                ...
                <input type="text" value={lastName} onChange={(e) => setLastName(e.target.value)} placeholder="e.g., Sharma"
                  className="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-brand-500 focus:ring-2 focus:ring-brand-100 outline-none transition-all" />
              </div>
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input type="tel" value={mobileNumber} onChange={(e) => setMobileNumber(e.target.value)} placeholder="98765 43210" maxLength={10}
          ...
          {step === 'otp' && (
            <div className="space-y-5">
              ...
              <div className="text-center">
                {resendTimer > 0 ? <span className="text-sm text-gray-400">Resend OTP in {resendTimer}s</span>
                : <button onClick={sendOtp} className="text-sm text-brand-600 hover:text-brand-700 font-medium">Resend OTP</button>}
              </div>
            </div>
          )}

          <div className="flex items-center justify-center gap-2 text-gray-400 text-xs mt-6">
            <Shield className="w-4 h-4" /> <span>Secure OTP authentication · Your data is protected</span>
          </div>
        </div>
      </div>
    </div>
